...
**与标准 MySQL 8.0 版本的兼容性**。虽然许多标准 MySQL 模式和应用程序可以使用 NDB Cluster 运行，但未修改的应用程序和数据库模式在使用 NDB Cluster 运行时可能会稍微不兼容或性能欠佳（请参阅[NDB Cluster的已知限制”](/MySQL参考手册/NDB集群/NDB集群概述/NDB集群的已知限制/NDB集群的已知限制.md))。大多数这些问题都可以克服，但这也意味着您不太可能切换现有的应用程序数据存储（例如，当前使用 MyISAM 或 InnoDB）来使用 NDB 存储引擎，而不允许模式、查询和应用程序的变化。在没有 NDB 支持的情况下编译的 mysqld（即，在没有 [-DWITH_NDB](https://dev.mysql.com/doc/refman/8.0/en/source-configuration-options.html#option_cmake_with_ndb) 或 [-DWITH_NDBCLUSTER_STORAGE_ENGINE](https://dev.mysql.com/doc/refman/8.0/en/source-configuration-options.html#option_cmake_with_ndbcluster_storage_engine) 的情况下构建）不能用作使用它构建的 mysqld 的直接替代品。
